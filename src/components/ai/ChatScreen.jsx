...
export default function AIScreenChat({ currentWord, nativeLang, onClose }) {
  ...
  const handleSubmit = () => {
    ...
    setMessage('');
    // Focus back after send (optional)
...
//   // Auto-focus and scroll on mount (optional)
//   useEffect(() => {
...
useEffect(() => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
      ...
      {/* 🌟 SINGLE ScrollView containing BOTH chat AND input */}
      <ScrollView
        ref={scrollViewRef}
        style={{ flex: 1 }}
        ...
        {messages.length === 0 ? (
          <View style={{ alignItems: 'center', marginTop: 20 }}>
            ...
            <View style={{ width: '100%', gap: 10, marginTop: 20 }}>
              {quickPrompts.map((prompt, index) => (
                ...
                >
                  <Text style={{ fontSize: 15, color: '#1f2937' }}>{prompt}</Text>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={{ gap: 1 }}>
            ...
            {isChatLoading && (
              ...
              >
                <Text style={{ color: '#4b5563' }}>Thinking...</Text>
              </View>
            )}

            {/* Extra space before input */}
            <View style={{ height: 20 }} />
          </View>
        )}
        

        {/* Extra bottom buffer for Yandex toolbar */}
        {/* <View style={{ height: 40 }} /> */}
      </ScrollView>
      {/* 💡 INPUT BAR INSIDE ScrollView */}
       <View
        style={{
          paddingHorizontal: 16,
          paddingVertical: 12,
          // 🔥 EXTRA padding on Android for Yandex toolbar
